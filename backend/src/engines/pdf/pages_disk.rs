...
/// Reorders PDF pages directly from a disk-backed input.
///
/// The source PDF stays on disk while lopdf builds the in-memory document graph.
/// Only the generated output is materialized as a `Vec<u8>` for the HTTP response.
pub fn manage_pages_from_path(
    pdf_path: &Path,
    page_order: &[u32],
...
mod tests {
    ...
    fn rejects_missing_input_path() {
        let result = manage_pages_from_path(
    ...
    fn rejects_zero_page_number() {
        let result = validate_page_order_for_test(&[0]);
        assert!(result.is_err());
    }

    fn validate_page_order_for_test(page_order: &[u32]) -> Result<(), String> {
        if page_order.is_empty() || page_order.len() > MAX_PAGE_ORDER_ITEMS || page_order.contains(&0) {
            return Err("invalid page order".to_owned());
        }
        Ok(())
    }
