...
use super::{compress::CompressionQuality, pdf_input};
use crate::engines::pdf::common::{load_pdf_document, load_pdf_document_from_path, validate_pdf_path};
...
/// Compress a PDF that is already stored on disk.
///
/// Keeping the upload disk-backed means the multipart parser does not retain the
/// full source PDF as a `Bytes` allocation while the conversion worker is queued.
pub fn compress_pdf_from_path(
    ...
    {
        let output_path = temp_dir.path().join(format!("pymupdf-{}.pdf", quality.as_str()));
        ...
        .is_ok()
        {
            if let Ok(output) = fs::read(&output_path)
                && is_valid_pdf(&output, input_pages)
            {
                ...
                {
                    return Ok(fallback.min_by_size(output));
                }
            }
        }
    }
...
trait MinBySize {
    fn min_by_size(self, other: Vec<u8>) -> Vec<u8>;
}

impl MinBySize for Vec<u8> {
    fn min_by_size(self, other: Vec<u8>) -> Vec<u8> {
...
mod pdf_input {
    // Marker module kept private so the new path-based engine has a stable namespace.
}
...
mod tests {
    ...
    #[test]
    fn path_validation_rejects_missing_file() {
