...
/** 6-digit hex for the OG card placeholder, or "" if site.color is missing/invalid. */
...
/**
 * "wild-race.pdfin.app" → "Wild Race". Only published app hosts encode the
 * display name in the first label.
 */
export function appNameFromHost(hostHeader) {
  const host = String(hostHeader ?? "")
    ...
    .split(":")[0]
    .toLowerCase();
  if (!host.endsWith(".pdfin.app") && host !== "pdfin.app") {
...
/**
 * Published apps always use `VITE_PUBLIC_HOSTNAME`. Live preview has no such
 * env, so fall back to the request host / X-Forwarded-Host.
 */
...
/** App extensions script  -  injected into every HTML document (no-op when EXTENSIONS_SCRIPT_SRC is empty). */
export function extensionsHeadTags(projectId = readProjectId()) {
  if (!EXTENSIONS_SCRIPT_SRC) return [];
  const id = escapeHtml(projectId);
  const tags = [];
  if (projectId) {
    tags.push(`<meta name="pdfin-project-id" content="${id}">`);
  }
  tags.push(
    `<script src="${EXTENSIONS_SCRIPT_SRC}"${
 ...
 * Otherwise empty  -  caller emits the og placeholder.
 */
export function resolveOgCardAsset(site = {}, cwd = process.cwd(), detectFs = true) {
...
export function injectPwaHead(html, ctx = {}) {
  ...
  if (
    projectId &&
    !next.includes('property="pdfin:app_id"') &&
    !next.includes("property='pdfin:app_id'")
  ) {
