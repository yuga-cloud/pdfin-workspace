...
test("injects the extensions script without a project id", () => {
  ...
  // EXTENSIONS_SCRIPT_SRC is empty string  -  no script injected
...
test("emits x:game:image for a public host when site.banner is set", () => {
  ...
  assert.match(
    out,
    /property="x:game:image" content="https:\/\/wild-race\.pdfin\.app\/x-banner\.jpg"/,
  );
...
test("emits og:image for a public host and prefers a custom card", () => {
  ...
  assert.match(
    placeholder,
    /property="og:image" content="https:\/\/og\.pdfin\.app\/v1\/card\.png\?host=wild-race\.pdfin\.app&amp;title=Wild%20Race"/,
  );
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  ...
  assert.match(
    themed,
    /property="og:image" content="https:\/\/og\.pdfin\.app\/v1\/card\.png\?host=wild-race\.pdfin\.app&amp;title=Wild%20Race&amp;color=FF4D2E"/,
  );
