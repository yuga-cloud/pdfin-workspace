...
import { processTool, revokeObjectUrls, type ToolOptions } from "@/lib/pdf/engine";
...
function getDropLabel(tool: ToolDef): string {
  switch (tool.accept) {
    ...
    case "pdf":
      return tool.multiple ? "Letakkan PDF di sini, atau pilih dari perangkat" : "Letakkan satu PDF di sini, atau pilih dari perangkat";
    default:
...
function ToolWorkspaceInner({ tool }: { tool: ToolDef }) {
  ...
  useEffect(() => {
    ...
    if (tool.accept !== "pdf" || items.length !== 1 || (tool.slug !== "pisah" && tool.slug !== "halaman")) {
      return;
    }
    ...
    return () => {
      cancelled = true;
      if (thumbCacheRef.current === cache) {
        thumbCacheRef.current = null;
      }
      cache.dispose();
    };
  ...
  return (
    <div className={cn("tool-workspace-page mx-auto max-w-3xl", `tool-processing-${processingLocation}`, `tool-accept-${tool.accept}`)}>
      <button type="button" onClick={handleBack} className="workspace-back-link mb-7 inline-flex items-center gap-2 rounded-lg px-1 py-1 text-sm font-medium text-muted transition-colors hover:text-fg">
      ...
      <div className="workspace-tool-heading">
        <div className="workspace-tool-heading-top">
          <p className="workspace-tool-group text-xs font-bold uppercase tracking-[0.14em] text-primary">{tool.group === "atur" ? "Atur" : tool.group === "optimalkan" ? "Optimalkan" : "Konversi"}</p>
          <span className="workspace-processing-pill">{processingLocation === "server" ? "Diproses di server" : "Diproses di perangkat"}</span>
        </div>
            ...
            {items.map((item, index) => (
              <li key={item.id} className="workspace-file-row flex items-center gap-3 rounded-lg bg-bg px-3 py-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted tabular-nums">{formatBytes(item.file.size)}{pageCount && items.length === 1 ? ` · ${pageCount} halaman` : ""}</p>
                </div>
                {tool.multiple ? (
                  <div className="workspace-inline-actions flex items-center gap-1">
                    <Button type="button" variant="ghost" size="icon" className="size-9 min-h-9" aria-label="Naikkan" onClick={() => move(item.id, -1)} disabled={index === 0}><ArrowUp /></Button>
                    <Button type="button" variant="ghost" size="icon" className="size-9 min-h-9" aria-label="Turunkan" onClick={() => move(item.id, 1)} disabled={index === items.length - 1}><ArrowDown /></Button>
                  </div>
                ) : null}
      ...
      {busy ? (
        <div className="workspace-progress mt-6">
          <div className="mb-2 flex items-center justify-between text-xs text-muted"><span>{progress.label}</span><span className="tabular-nums">{Math.round(pct)}%</span></div>
          <Progress value={pct} />
        </div>
      ) : null}
    ...
    <section className="workspace-options mt-6 rounded-2xl bg-surface p-4 sm:p-5">
      ...
      {tool.slug === "pisah" ? (
        <div className="mt-4 space-y-4">
          <label className="workspace-check-row flex items-center gap-3 text-sm"><input type="checkbox" className="size-4 accent-primary" checked={options.splitEach} onChange={(event) => setOptions((current) => ({ ...current, splitEach: event.target.checked }))} />Pecah tiap halaman jadi file PDF terpisah (ZIP)</label>
          {options.splitEach ? null : <label className="block text-sm"><span className="text-muted">Rentang halaman {pageCount ? `(1–${pageCount})` : ""}</span><input value={options.rangeText} onChange={(event) => setOptions((current) => ({ ...current, rangeText: event.target.value }))} placeholder="1-3, 5, 8-10" className="workspace-field mt-1 h-11 w-full rounded-md border border-border bg-bg px-3 text-sm outline-none" /></label>}
        </div>
      ) : null}
        ...
        <fieldset className="mt-4">
          ...
          <div className="workspace-option-grid mt-3 grid gap-2 sm:grid-cols-3">
            {([["high", "Halus", "Lebih tajam, file lebih besar"], ["medium", "Sedang", "Seimbang, disarankan"], ["low", "Kecil", "Paling hemat, untuk WhatsApp"]] as const).map(([value, label, hint]) => (
              <label key={value} className={cn("workspace-choice cursor-pointer rounded-lg border px-3 py-3 text-sm", options.quality === value && "is-selected")}><input type="radio" name="quality" className="sr-only" checked={options.quality === value} onChange={() => setOptions((current) => ({ ...current, quality: value }))} /><span className="font-semibold">{label}</span><span className="mt-1 block text-xs text-muted">{hint}</span></label>
            ))}
          </div>
        </fieldset>
      ) : null}
      ...
      {tool.slug === "putar" ? (
        <fieldset className="mt-4"><legend className="text-sm text-muted">Putar semua halaman</legend><div className="workspace-segmented mt-3 flex flex-wrap gap-2">{([90, 180, 270] as const).map((degrees) => <button key={degrees} type="button" onClick={() => setOptions((current) => ({ ...current, rotateDegrees: degrees }))} className={cn("h-11 rounded-md px-4 text-sm", options.rotateDegrees === degrees && "is-selected")}>{degrees}°</button>)}</div></fieldset>
      ) : null}

      {tool.slug === "watermark" ? (
        <div className="mt-4 space-y-4">
          <label className="block text-sm"><span className="text-muted">Teks stempel</span><input value={options.watermarkText} onChange={(event) => setOptions((current) => ({ ...current, watermarkText: event.target.value }))} placeholder="DRAFT, ASLI, RAHASIA" maxLength={48} className="workspace-field mt-1 h-11 w-full rounded-md border border-border bg-bg px-3 text-sm outline-none" /></label>
          <div className="workspace-chip-row flex flex-wrap gap-2">{["DRAFT", "ASLI", "RAHASIA", "KOPI"].map((watermark) => <button key={watermark} type="button" className="workspace-chip h-9 rounded-full px-3 text-xs font-medium" onClick={() => setOptions((current) => ({ ...current, watermarkText: watermark }))}>{watermark}</button>)}</div>
          <label className="block text-sm"><span className="flex items-center justify-between gap-4 text-muted"><span>Ketebalan stempel</span><span className="tabular-nums text-fg">{Math.round(options.watermarkOpacity * 100)}%</span></span><input type="range" min={0.08} max={0.4} step={0.02} value={options.watermarkOpacity} onChange={(event) => setOptions((current) => ({ ...current, watermarkOpacity: Number(event.target.value) }))} className="mt-2 w-full accent-primary" /></label>
          <label className="workspace-check-row flex items-center gap-3 text-sm"><input type="checkbox" className="size-4 accent-primary" checked={options.addPageNumbers} onChange={(event) => setOptions((current) => ({ ...current, addPageNumbers: event.target.checked }))} />Tambahkan nomor halaman</label>
        </div>
      ) : null}
      ...
      {(tool.slug === "pdf-ke-jpg" || tool.slug === "word-ke-pdf" || tool.slug === "powerpoint-ke-pdf" || tool.slug === "pdf-ke-word" || tool.slug === "pdf-ke-excel" || tool.slug === "pdf-ke-powerpoint" || tool.slug === "excel-ke-pdf") ? (
        <div className="workspace-info-note mt-4 rounded-xl bg-bg px-3 py-3 text-xs leading-relaxed text-muted">Mode pemrosesan: {processingLocationLabel(tool, options)}. Detail dan batasan hasil mengikuti alat yang kamu pilih.</div>
      ) : null}
    </section>
  );
}
