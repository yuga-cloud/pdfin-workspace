...
import { compressPdf as compressPdfApi } from "@/lib/api/optimize";
...
import { blobFromBytes, stem, yieldToUi } from "@/lib/pdf/bytes";
...
async function normalizeImageToJpeg(file: File): Promise<Blob> {
  ...
  if (typeof createImageBitmap !== "function") fail("Browser tidak mendukung konversi format gambar ini.");
  ...
  try {
    ...
    if (!context) fail("Browser tidak mendukung canvas.");
    context.drawImage(bitmap, 0, 0);
    return await new Promise<Blob>((resolve, reject) => {
      canvas.toBlob(
        (blob) => (blob ? resolve(blob) : reject(new Error("Gagal mengonversi gambar ke JPEG."))),
        "image/jpeg",
        0.92,
      );
    });
...
export function parsePageRange(text: string, pageCount: number): number[] {
  ...
  if (indexes.size === 0) fail("Tidak ada halaman yang cocok dengan rentang itu.");
...
export async function renderThumbs(file: File, onProgress?: ProgressFn, signal?: AbortSignal): Promise<string[]> {
  ...
  try {
    for (let pageNumber = 1; pageNumber <= total; pageNumber += 1) {
      throwIfAborted(signal);
      onProgress?.(pageNumber - 1, total, `Pratinjau halaman ${pageNumber}/${total}`);
      ...
      const blob = await new Promise<Blob>((resolve, reject) => {
        canvas.toBlob((value) => {
          ...
          else reject(new Error("Gagal membuat pratinjau JPG."));
        }, "image/jpeg", 0.72);
      });
...
async function splitEachPage(file: File, onProgress?: ProgressFn): Promise<Blob> {
  const { PDFDocument } = await loadPdfLib();
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    zip.file(`${base}-halaman-${String(index + 1).padStart(3, "0")}.pdf`, bytes);
    await yieldToUi();
  }
...
async function splitSelectedPages(file: File, rangeText: string, pageCount: number, onProgress?: ProgressFn): Promise<Blob> {
  ...
  ranges.push({ start: start + 1, end: previous + 1 });
  const rangeValue = ranges.map((range) => (range.start === range.end ? String(range.start) : `${range.start}-${range.end}`)).join(",");
...
async function manageAllPages(file: File, pageOrder: number[], onProgress?: ProgressFn): Promise<Blob> {
...
async function rotateAll(file: File, degrees: 90 | 180 | 270, onProgress?: ProgressFn): Promise<Blob> {
...
async function stampPdf(file: File, options: { text: string; opacity: number; numbers: boolean }, onProgress?: ProgressFn): Promise<Uint8Array> {
...
export async function processTool(slug: ToolSlug, files: File[], options: ToolOptions, onProgress?: ProgressFn): Promise<ProcessOutput> {
  const primary = files[0];
  if (!primary) fail("Pilih file dulu.");
  switch (slug) {
    case "gabung": {
    ...
    case "pdf-ke-jpg": {
      ...
      try {
        for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
          ...
          zip.file(`${stem(primary.name)}-${String(pageNumber).padStart(3, "0")}.jpg`, blob);
          canvas.width = 0;
          canvas.height = 0;
          page.cleanup();
          await yieldToUi();
        ...
        return { blob, filename: `${stem(primary.name)}-jpg.zip`, mime: "application/zip" };
      } finally {
        await pdf.cleanup();
      }
    }
    case "watermark": {
      const bytes = await stampPdf(primary, { text: options.watermarkText, opacity: options.watermarkOpacity, numbers: options.addPageNumbers }, onProgress);
    ...
    case "excel-ke-pdf": return excelToPdfApi(primary);
    ...
    case "pdf-ke-excel": return pdfToExcelApi(primary);
